...
class Colecciones_model extends CI_Model {
	...
	public function ListarPorIdAcc($id){
		...
		$resultados = $this->db->query("CALL SPU_COLECCIONES_PORIDUREG('".$id."')");  
		$this->db->free_db_resource();
		return $resultados->result();
	}
	public function guardar($nombre,$descripcion,$imagen,$activo,$uregistro){

		$resultados = $this->db->query("CALL SPU_COLECCIONES_GUARDAR('".$nombre."','".$descripcion."','aun no','".$activo."',".$uregistro.")");  
		$this->db->free_db_resource();
